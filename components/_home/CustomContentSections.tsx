...
import { useHeroContent } from '@/hooks/useHome';
import { motion, AnimatePresence } from 'framer-motion';
import { ChevronDown, ArrowRight, HelpCircle } from 'lucide-react';
import Link from 'next/link';

export const CustomContentSections = () => {
  const { data: homeContent, isLoading } = useHeroContent();
  ...
  if (isLoading || !homeContent || !homeContent.customSections || homeContent.customSections.length === 0) {
    return null;
  }
  ...
  return (
    <div className="space-y-24 py-20 bg-gradient-to-b from-slate-50/50 to-white overflow-hidden">
      {sections.map((section: any, sIdx: number) => {
        ...
        return (
          <section key={sIdx} className="max-w-7xl mx-auto px-4 md:px-8">
            <div className={`grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center ${
              isEven ? '' : 'lg:flex-row-reverse'
            }`}>
              
              {/* Text & Content Block (col-span-7 or full if no images) */}
              ...
              {/* Images Block (col-span-5 or hidden if no images) */}
              {hasImages && (

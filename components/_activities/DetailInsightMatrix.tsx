            ...
            {localInfo.famousPlaces.map((p: any, i: number) => (
              <div key={i} className="flex flex-col sm:flex-row justify-between sm:items-center bg-slate-50 p-3.5 rounded-xl border border-slate-100/70 gap-2">
                ...
                <div className="text-right flex-shrink-0 text-xs font-bold text-slate-700">
                  <span>Fee: {p.entryFee > 0 ? `₹${p.entryFee}` : 'Free Entry'}</span>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {localInfo.clothing.summer?.length > 0 && (
                <div>
                  ...
                  <p className="text-slate-500">{localInfo.clothing.summer.join(', ')}</p>
                </div>
              )}
              {localInfo.clothing.religiousPlaces?.length > 0 && (
                <div>
                  <span className="font-black text-rose-900 block mb-1">Temples & Holy Dresscodes</span>
                  <p className="text-slate-500">{localInfo.clothing.religiousPlaces.join(', ')}</p>
                </div>
              )}
            </div>
          </div>

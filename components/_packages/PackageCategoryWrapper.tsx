...
interface PackageCategoryWrapperProps {
...
export const PackageCategoryWrapper = ({
  id,
  title,
  subtitle,
  icon,
  ...
  onScrollLeft,
  onScrollRight,
}: PackageCategoryWrapperProps) => {

  const themeStyles: Record<PackageCategoryId, {
    sectionBg: string;
    titleColor: string;
    subColor: string;
    ...
    // ⚡ Epic Adventures — Orange/Red active volcano energy
    adventure: {
    ...
    // 🌹 Romantic Escapes — Soft rose pink garden
    romantic: {
    ...
    // 🌿 Family Holidays — Forest green canopy
    family: {
    ...
    // 💎 Premium Luxury — Deep slate gold & blue glow
    luxury: {
    ...
    // 🪷 Spiritual Retreats — Violet & lavender sanctuary
    spiritual: {
    ...
    // 🏛️ Heritage & Culture — Amber/parchment serifs
    cultural: {
  ...
  const currentTheme = themeStyles[id] || themeStyles.luxury;

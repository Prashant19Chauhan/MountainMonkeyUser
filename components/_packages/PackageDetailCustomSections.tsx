...
import { usePackageDetailSections } from '@/hooks/usePackages';
...
export const PackageDetailCustomSections = ({ packageSlug }: Props) => {
  const { data: sectionsData, isLoading } = usePackageDetailSections(packageSlug);
  ...
  return (
    <section className="max-w-8xl mx-auto px-4 md:px-12 lg:px-24 pb-20 space-y-24">
                ...
                <div className={`flex flex-wrap gap-4 pt-2 ${!hasImages ? 'justify-center' : ''}`}>
                  {section.links.map((link: any, lIdx: number) => (
                    <Link key={lIdx} href={link.url} className="no-underline">
                      <button className="flex items-center gap-2 px-6 py-3.5 bg-slate-900 hover:bg-emerald-600 text-white rounded-full font-black text-[10px] md:text-xs uppercase tracking-widest transition-all hover:shadow-lg hover:shadow-emerald-100 border-0 cursor-pointer duration-300">
                        {link.text}
                        <ArrowRight size={13} />
                      </button>
                    </Link>
